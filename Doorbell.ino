...
int triggerPins[TRIGGER_COUNT] = {0, 1, 5, 10, A4, A3, A2, A1, A0};
...
void loop()
{
  for (int i=0; i<TRIGGER_COUNT; i++)
  {
    if ((digitalRead(triggerPins[i]) == LOW) && ((millis()-lastTrigger)>SONGLENGTH))
    {
      ...
      //uint8_t result = 
      MP3player.playTrack((i*10)+randomNumber);
...
void initMP3Player()
{
  ...
  {
    //Serial.println("Something went wrong");  //Lol, no serial!
